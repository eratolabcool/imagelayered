...
CREATE TABLE `lookbook_character` (
	`id` text PRIMARY KEY NOT NULL,
	`user_id` text NOT NULL,
	`name` text DEFAULT 'My Fashion Persona' NOT NULL,
...
--> statement-breakpoint
CREATE INDEX `idx_lookbook_character_user_id` ON `lookbook_character` (`user_id`);--> statement-breakpoint
CREATE INDEX `idx_lookbook_character_created_at` ON `lookbook_character` (`created_at`);--> statement-breakpoint
...
CREATE INDEX `idx_subscriber_email` ON `newsletter_subscriber` (`email`);--> statement-breakpoint
CREATE INDEX `idx_subscriber_created_at` ON `newsletter_subscriber` (`created_at`);--> statement-breakpoint
CREATE TABLE `order` (
	`id` text PRIMARY KEY NOT NULL,
...
CREATE INDEX `idx_project_created_at` ON `project` (`created_at`);--> statement-breakpoint
CREATE TABLE `role` (
	`id` text PRIMARY KEY NOT NULL,
	`name` text NOT NULL,
	`title` text NOT NULL,
...
CREATE TABLE `studio_guest_usage` (
	`id` text PRIMARY KEY NOT NULL,
	`actor_key` text NOT NULL,
	`day_key` text NOT NULL,
	`ai_operations` integer DEFAULT 0 NOT NULL,
	`created_at` integer DEFAULT (cast((julianday('now') - 2440587.5)*86400000 as integer)) NOT NULL,
	`updated_at` integer DEFAULT (cast((julianday('now') - 2440587.5)*86400000 as integer)) NOT NULL
);
--> statement-breakpoint
CREATE INDEX `idx_studio_guest_usage_actor_day` ON `studio_guest_usage` (`actor_key`,`day_key`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX `idx_studio_operation_project_created` ON `studio_operation` (`project_id`,`created_at`);--> statement-breakpoint
CREATE INDEX `idx_studio_operation_actor_status` ON `studio_operation` (`actor_key`,`status`);--> statement-breakpoint
CREATE INDEX `idx_studio_operation_ai_task` ON `studio_operation` (`ai_task_id`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX `idx_studio_revision_project_created` ON `studio_revision` (`project_id`,`created_at`);--> statement-breakpoint
CREATE INDEX `idx_studio_revision_actor` ON `studio_revision` (`actor_key`);